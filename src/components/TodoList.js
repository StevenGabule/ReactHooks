        ...
        <div className={"container mx-auto max-w-md text-center font-mono"}>
            ...
            <ul className={"list-reset text-white p-0"}>
                {state.todos.map(todo => (
                    <li key={todo.id} className={"flex items-center bg-orange-600 border-black border-dashed border-2 my-2 py-4"}>
                        <span className={`flex-1 cursor-pointer ${todo.complete && "line-through text-grey-100"}`} onDoubleClick={() => dispatch({type: "TOGGLE_TODO", payload: todo})}>
                            {todo.text}
                        </span>
