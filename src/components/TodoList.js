            ...
            <ul className={"list-reset text-white p-0"}>
                {state.todos.map(todo => (
                    <li key={todo.id}
                        ...
                        <span className={`flex-1 cursor-pointer ${todo.complete && "line-through text-grey-100"}`}
                              onDoubleClick={async () => {
                                  const response = await axios.patch(`https://hooks-api-glx465q80.now.sh/todos/${todo.id}`, {
                        ...
                        <button onClick={async () => {
                            await axios.delete(`https://hooks-api-glx465q80.now.sh/todos/${todo.id}`);
